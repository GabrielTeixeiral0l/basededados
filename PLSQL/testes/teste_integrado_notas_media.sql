...
DECLARE
    ...
    v_doc_id    NUMBER;
    v_cur_id    NUMBER;
    v_uc1_id    NUMBER; -- UC de 10 ECTS
    v_uc2_id    NUMBER; -- UC de 20 ECTS
    v_est_id    NUMBER;
    v_mat_id    NUMBER;
    v_tur1_id   NUMBER;
    v_tur2_id   NUMBER;
...
BEGIN
    ...
    -- 1. SETUP DE DADOS
    -- Criar Docente e Tipo de Curso
    INSERT INTO docente (nome, nif, email, telemovel, data_contratacao, status)
    VALUES ('Doc Integ '||v_sfx, '4'||v_sfx||'4444', 'di'||v_sfx||'@t.pt', '9'||v_sfx||'4444', SYSDATE, '1') RETURNING id INTO v_doc_id;
    
    INSERT INTO tipo_curso (nome, valor_propinas) VALUES ('Tipo Demo', 1000) RETURNING id INTO v_cur_id; -- Reuso var
    ...
    VALUES (v_cur_id, v_uc1_id, 1, 1, 10, '1');

    -- UC 2: Peso alto (20 ECTS)
    INSERT INTO unidade_curricular (nome, codigo, horas_teoricas, horas_praticas) 
    VALUES ('UC2 '||v_sfx, 'U2'||v_sfx, 30, 30) RETURNING id INTO v_uc2_id;
    INSERT INTO uc_curso (curso_id, unidade_curricular_id, semestre, ano, ects, presenca_obrigatoria) 
    VALUES (v_cur_id, v_uc2_id, 1, 1, 20, '1');

    -- Turmas
    INSERT INTO turma (nome, ano_letivo, unidade_curricular_id, docente_id) 
    VALUES ('T1', '2025', v_uc1_id, v_doc_id) RETURNING id INTO v_tur1_id;
    INSERT INTO turma (nome, ano_letivo, unidade_curricular_id, docente_id) 
    VALUES ('T2', '2025', v_uc2_id, v_doc_id) RETURNING id INTO v_tur2_id;

    -- Aluno e Matrícula
    INSERT INTO estudante (nome, nif, cc, data_nascimento, email, telemovel) 
    VALUES ('Aluno Integ '||v_sfx, '5'||v_sfx||'5555', '1'||v_sfx||'555', SYSDATE-7000, 'ai'||v_sfx||'@n.pt', '9'||v_sfx||'5555') 
    ...
    VALUES ('Aval UC1', SYSDATE, SYSDATE, 100, 1, v_tur1_id, v_tav_id) RETURNING id INTO v_aval1_id;

    INSERT INTO avaliacao (titulo, data, data_entrega, peso, max_alunos, turma_id, tipo_avaliacao_id)
    VALUES ('Aval UC2', SYSDATE, SYSDATE, 100, 1, v_tur2_id, v_tav_id) RETURNING id INTO v_aval2_id;

    -- 2. LANÇAMENTO DE NOTAS PARCIAIS
    -- Nota 18 na UC1 (10 ECTS)
    INSERT INTO nota (inscricao_id, avaliacao_id, nota, comentario) VALUES (v_ins1_id, v_aval1_id, 18, 'Muito Bom');
    
    -- Nota 12 na UC2 (20 ECTS)
    ...
    -- Simular que o professor fechou a pauta e a nota final é igual à nota da avaliação única (simplificado)
